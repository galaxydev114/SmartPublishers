        ...
        <div class="container">
            <div class="row">
                <div class="col-12">
                    ...
                    <label class="ml-1">{{ __('globals.common.site_campaign') }}: </label>
                    {!! Form::select('cmp_site_id', [], '', array('id'=>'cmp_site_id', 'class'=> 'list-inline', 'multiple' => 'multiple')) !!}
                    <button id="chart_view" class="btn btn-secondary waves-effect waves-light btn-sm list-inline"><i class="mdi mdi-eye"></i> </button>
                    ...
                    <div class="card m-b-20">
                        <div class="card-block">
                            <h4 class="mt-0 header-title list-inline">
                                <label id="sheet_title" data-id="-1">{{ __('globals.common.sheet_analysis') }}
                                </label>
                                <input class="mr-2" type="checkbox" id="apex_chart_visible"  />
                                <a class="mt-0 list-inline float-right" id="btn_back" href="{{ route('sheet.index') }}"><i class="mdi mdi-arrow-left"></i> {{ __('globals.common.back') }}</a>
                            </h4>
                            <div class="p-3 effect8" id="apex_chart">
                                <div id="chart-cmp-site" height="120"></div>
                            </div>
                            <div class="row">
...
@push('scripts')
    ...
    <script>
        ...
        $(function() {
            ...
            cmp_ids = $("#cmp_site_id").filterMultiSelect({
                placeholderText: "{{ __('globals.common.select_items') }}",
                filterText: "{{ __('globals.common.filter') }}",
                selectAllText: "{{ __('globals.common.select_all') }}",
                caseSensitive: false,
                allowEnablingAndDisabling: true,
                items: [],
            });

            apexVisible = localStorage.getItem('apex_visible') ?? 'hide';

            if(apexVisible == 'show')
            {
                $('#apex_chart_visible').attr('checked', '');
                $('#apex_chart').show();
                cmp_ids.enable();
            } else {
                $('#apex_chart').hide();
                $('#apex_chart_visible').removeAttr('checked');
                cmp_ids.disable();
            }

            new Switchery($('#apex_chart_visible')[0], {size:'small', color: '#0D74E9'});

            $('#apex_chart_visible').on('change', function(evt) {
                if (this.checked) {
                    localStorage.setItem('apex_visible', 'show');
                    apexVisible = 'show';
                    cmp_ids.enable();
                    $('.select2-container').css('top', '552px');
                    $('#apex_chart').show();
                } else
                {
                    localStorage.setItem('apex_visible', 'hide');
                    $('#apex_chart').hide();
                    apexVisible = 'hide';
                    cmp_ids.disable();
                    $('.select2-container').css('top', '85px');
                }
            });
            ...
            let options = {
                ...
                yaxis: [{
                    title: {
                        text: "{{ __('globals.common.matrix') }}"
                    }
                }]
            };

            m_apexChart = new ApexCharts(document.querySelector('#chart-cmp-site'), options);
            ...
            $('#chart_view').click(function () {
                let selectedOptions = JSON.parse(cmp_ids.getSelectedOptionsAsJson()).cmp_site_id;
                if(selectedOptions.length === 0) {
                    toastr.warning("{{ __('globals.msg.please_select_displayed') }}", "{{ __('globals.msg.warning') }}");
                    return false;
                }
                ...
                let indexList = [];
                selectedOptions.map(function (el) {
                    indexList.push(m_apexAllLabels.indexOf(el));
                });

                for (const [key, value] of Object.entries(m_apexDataList)) {
                    let newData = value.data;
                    let newItems = [];

                    indexList.map(function (el) {
                        newItems.push(value.data[el]);
                    })
        ...
        function sheetView(cstart, cend)
        {
            ...
            $.ajax({
                ...
                success : function(res) {
                    ...
                    // Init Multi Select drop down
                    //let selectedOptions = cmp_ids.getSelectedOptionsAsJson();
                    //console.log(JSON.parse(selectedOptions).cmp_site_id);
                    //localStorage.setItem('cur_site_cmp_ids', selectedOptions);

                    if(res.status)
                    {
                        $('#cmp_site_id').replaceWith(`{!! Form::select('cmp_site_id', [], '', array('id'=>'cmp_site_id', 'class'=> 'list-inline', 'multiple' => 'multiple')) !!}`);
                        let items = [];
                        let apexColors = [];
                        let apexSeries = [];
                        let apexWidth = [];

                        res.apexlabels.map(function (el) {
                            items.push([el, el]);
                            apexColors.push(getRandomColor());
                            apexWidth.push(2);
                        });
                        ...
                        for (const [key, value] of Object.entries(res.apexseries)) {
                            apexSeries.push({
                                name: key,
                                data: value
                            });
                        ...
                        m_apexDataList = apexSeries;
                        m_apexAllLabels = res.apexlabels;

                        m_apexChart.updateOptions({
                            series: apexSeries,
                            labels: res.apexlabels,
                            colors: apexColors,
                            stroke: {
                                width: apexWidth,
                                curve: 'smooth'
            ...
            $.ajax({
                ...
                success : function(res) {
                    if (res.status)
                    {
                        ...
                        $('#th_bounce_rate').remove();
                        $('#th_page_session').remove();
                        $('#th_avg_duration').remove();
                        $('#th_ctr').remove();
                        $('#th_ecpm').remove();
                        ...
                        dtable.column( 11 ).visible( false, false ); //Bix MAX
                        dtable.column( 12 ).visible( false, false ); //Profit Min
                        dtable.column( 13 ).visible( false, false ); //Profit Min
                        dtable.column( 14 ).visible( false, false ); //Profit Min
