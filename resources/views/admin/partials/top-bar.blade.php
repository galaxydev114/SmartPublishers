<div class="topbar">
    ...
    <nav class="navbar-custom">
        ...
        <ul class="list-inline float-right mb-0">
            ...
            @if(\DLW\Models\Report::isTrialExpried() || Auth::guard('admin')->user()->is_super)
                <li class="list-inline-item">
                    ...
                    <span id="span_balance" class="@if(session()->get('cur_balance') < 100) text-danger @else text-success @endif"
                          ...
                          data-trigger="hover"
                          data-content="{{ __('globals.msg.balance_limit') }}">R$ {{ number_format(session()->get('cur_balance'), 2, '.', ',') }}
                    </span>
                </li>
            @endif
            <li class="list-inline-item dropdown notification-list">
                ...
                <div class="dropdown-menu dropdown-menu-right profile-dropdown ">
                    <a class="dropdown-item" href="{{ route('admin.profile', Auth::guard('admin')->user()->id) }}"><i class="dripicons-user text-muted"></i> {{ __('globals.common.my_account') }}</a>
...
{{--                    @endif--}}
                    @if(!Auth::guard('admin')->user()->is_super && session()->get('permissions') && array_key_exists('subscription_page', session()->get('permissions')->toArray()) && session()->get('permissions')->toArray()['subscription_page'] == 1)
                        <a class="dropdown-item" href="{{ route('subscription.index') }}"><i class="dripicons-rocket text-muted"></i> {{ __('globals.subscription.subscription') }}</a>
                    @endif
                    <div class="dropdown-divider"></div>
                    <a class="dropdown-item" href="#" onclick="to_logout()"><i class="mdi mdi-logout m-r-5 text-muted"></i> {{ __('globals.common.logout') }}</a>
                      @push('scripts')
                        <script>
                            function to_logout(){
                                $('#logout-form').submit();
