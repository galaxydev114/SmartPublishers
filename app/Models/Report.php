...
//        $response = json_decode($result, true);
    ...
    static function getTotalBalance($payhistory)
    {
        ...
        foreach ($payhistory as $row) {
            if(floatval($row['spent']) == 0) continue;
    ...
    static function bulkUpdateTaboolaCampaigns($value)
    {
        $account_id = session('account_name');
        $access_token = session('access_token');
        $base_url = env('TO_API_BASE_URL');
    ...
    static function findSite($data, $site, $siteid)
    {
    ...
    static function findSiteAll($data, $site, $siteid)
    {
        $res = [];
        foreach ($data as $key => $value) {
            if (preg_match("/\b$site\b/i", $value[0]) || preg_match("/\b$siteid\b/i", $value[0]))
    ...
    static function isTrialExpried()
    {
        $admin = Auth::guard('admin')->user();
        if ($admin->trial_end < date('Y-m-d') && $admin->is_subscribed != 1 && !$admin->is_super)
            return true;
        return false;
    }

    static function allCmpPause()
    {
        ////////////// All Campaign Puase Balance < 100 and Trail Expired //////////////////////////////////
        ...
        if((session()->get('cur_balance') < 100 && Report::isTrialExpried()) && !Auth::guard('admin')->user()->is_super)
        {
            ...
            foreach($allCampaigns as $key => $value)
            {
                if(array_key_exists('is_active', $value))
                {
                    if($value['is_active'])
                    {
                        $campaignsIds[] = $value['id'];
                        //$allCampaigns[$key] = Report::updateTaboolaCampaigns($value['id'], $sendVal);
                        $allCampaigns[$key]['is_active'] = false;
                    }
                }
            }
            if(sizeof($campaignsIds) > 0)
            {
                $sendCmpVal = [
                    'campaigns' => $campaignsIds,
                    'update' => $sendVal
                ];
                Report::bulkUpdateTaboolaCampaigns($sendCmpVal);
