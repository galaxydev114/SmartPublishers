...
class AdminsController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('admin.admins.index', ['title'=> __('globals.common.user_list'), 'admins'=>$admins]);
    }

    public function create()
    {
        return view('admin.admins.create', ['title'=>__('globals.common.create_account')]);
    }
    ...
    public function store(CreateadminRequest $request)
    {
        ...
        $site_ranking_page = $request->site_ranking_page ?? 0;
        $show_total_row = $request->show_total_row ?? 0;
        $subscription_page = $request->subscription_page ?? 0;
        ...
        $site_ranking_page = $site_ranking_page === 'on' ? 1 : 0;
        $show_total_row = $show_total_row === 'on' ? 1 : 0;
        $subscription_page = $subscription_page === 'on' ? 1 : 0;
        ...
        ClientSetting::query()->updateOrInsert(['user_id' => $user_id, 'page_key' => 'site_ranking_page'], ['user_id' => $user_id, 'page_key' => 'site_ranking_page', 'show_rule' => $site_ranking_page, 'created_at' => date("Y-m-d H:i:s"), 'updated_at' => date("Y-m-d H:i:s")]);
        ClientSetting::query()->updateOrInsert(['user_id' => $user_id, 'page_key' => 'show_total_row'], ['user_id' => $user_id, 'page_key' => 'show_total_row', 'show_rule' => $show_total_row, 'created_at' => date("Y-m-d H:i:s"), 'updated_at' => date("Y-m-d H:i:s")]);
        ClientSetting::query()->updateOrInsert(['user_id' => $user_id, 'page_key' => 'subscription_page'], ['user_id' => $user_id, 'page_key' => 'subscription_page', 'show_rule' => $subscription_page, 'created_at' => date("Y-m-d H:i:s"), 'updated_at' => date("Y-m-d H:i:s")]);
    ...
    {
        $title = __('globals.common.edit_account');
        ...
        if(sizeof($permissions) === 0)
        {
            ...
            $site_ranking_page = $permissions['site_ranking_page'] ?? 0 === 1 ? true : false;
            $show_total_row = $permissions['show_total_row'] ?? 0 === 1 ? true : false;
            $subscription_page = $permissions['subscription_page'] ?? 0 === 1 ? true : false;
        }

        $currency_min = $admin->currencies[0]->min_value ?? 4.2;
        $currency_max = $admin->currencies[0]->max_value ?? 4.2;

        return view('admin.admins.edit', compact('title', 'admin', 'currency_min', 'currency_max', 'report_page', 'show_total_row', 'site_ranking_page', 'campaign_management_page', 'campaign_page', 'column_visibility', 'currency_setting', 'ads_page', 'financial_setting', 'payment_history', 'content_page', 'utm_generator', 'subscription_page'));
    }
    ...
    public function profileSetting($userid)
    {
        ...
        return view('admin.admins.profile', compact('title', 'admin'));
    }
    ...
    public function update(UpdateadminRequest $request, Admin $admin)
    {
        ...
        $show_total_row = $show_total_row === 'on' ? 1 : 0;
        $subscription_page = $subscription_page === 'on' ? 1 : 0;
        if($subscription_page === 0)
        {
            $admin->is_subscribed = 0;
            $admin->subscribe_id = 0;
            $admin->agreement_id = "";
            $admin->plan_id = "";
            $admin->card_brand = "";
            $admin->card_last_four = "";
            $admin->trial_start = "";
            $admin->trial_end = "";
