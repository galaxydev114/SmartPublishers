...
Route::post('/stripe/webhook', 'Admin\StripeWebHookController@handleWebhook');
Route::post('/paypal/webhook', 'Admin\PaypalWebHookController@handleWebhook');
...
Route::prefix('admin')->group(function () {
    ...
    Route::get('password/reset/{token}', 'Admin\Auth\LoginController@showResetForm')->name('admin.show_reset_form');
    Route::post('password/reset', 'Admin\Auth\LoginController@resetPassword')->name('admin.reset_password');
    ...
    Route::get('/subscribe/paypal/return', 'Admin\PaypalController@paypalReturn')->name('paypal.return');
    ...
    Route::middleware(['admin.guard'])->group(function () {
        ...
        Route::resource('/site_ranking', 'Admin\SiteRankingController');
        Route::post('/site_ranking/datatable', 'Admin\SiteRankingController@datatable')->name('site_ranking.datatable');

        Route::resource('/subscription_list', 'Admin\SubscriptionListController');
        Route::post('/subscription_list/get_all', 'Admin\SubscriptionListController@ajaxGetAllData')->name('subscription_list.get_all');
        Route::post('/subscription_list/save_data', 'Admin\SubscriptionListController@ajaxSaveData')->name('subscription_list.save_data');
        Route::post('/subscription_list/edit_data', 'Admin\SubscriptionListController@ajaxEditData')->name('subscription_list.edit_data');
        Route::post('/subscription_list/remove/{item}', 'Admin\SubscriptionListController@ajaxItemRemove')->name('subscription_list.remove');
        Route::post('/subscription_list/session_subtype', 'Admin\SubscriptionListController@ajaxSetSessionSubType')->name('subscription.ajax_setsessionsubtype');

        Route::resource('/subscription', 'Admin\SubscriptionController');

        Route::get('create_paypal_plan/{subscribePlan}', 'Admin\PaypalController@create_plan')->name('paypal.create_plan');
        Route::get('create_paypal_webhook', 'Admin\PaypalWebHookController@registerPaypalWebHook')->name('paypal.create_webhook');

        Route::get('create_stripe_plan/{plan}', 'Admin\PlanController@create_stripe_plan')->name('stripe.create_plan');
        Route::post('subscribe/stripe/{plan}', 'Admin\SubscriptionPlanController@create')->name('stripe.plans');
        Route::get('subscribe/stripe/cancel', 'Admin\SubscriptionPlanController@cancel')->name('stripe.cancel');

        Route::get('subscribe/paypal/{subscribePlan}', 'Admin\PaypalController@paypalRedirect')->name('paypal.redirect');
        Route::get('subscribe/paypal_plan/cancel', 'Admin\PaypalController@paypalCancel')->name('paypal.cancel');
        Route::get('plans', 'Admin\PlanController@index')->name('plans.index');

        Route::get('admin/subscriptions', 'Admin\PaypalController@subscriptions')->name("user.subscriptions");
